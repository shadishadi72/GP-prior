...
load('dataset_pain_01234.mat')
no_sub=87;
dataset_hrveda=zeros(no_sub*2,size(dataset_pain_01234,2)+1);

dataset_hrveda(:,1:end-1)=dataset_pain_01234([87+1:2*87,4*87+1:end],:);
dataset_hrveda(1:87,end)=1;
dataset_hrveda(88:end,end)=-1;
...
for ii=1:max(subj)
    ...
    X_train=trainingSet(:,:);
       ...
       [X_train(:,1:end-1),mu,sigma]=zscore(trainingSet(:,1:end-1));
...
%     
    ...
    %  % adding features in priro
     meanfunc = @simple_feature_phasic_orig;
%           hyp.mean=log(ones(size(X_train,2)-1,1));
      no_feat=8;
...
%    
%      pred_label(exp(lp) > 0.51 & exp(lp) <0.9999 ) = 1; %% rest
...
% hold on; plot(t,xx','color','k','linewidth',2);
     ...
     feature_importance_all{ii,1}= feature_importance;
