...
if strcmp(gp_training_opts.kernel,'sqe')
    ...
    if gp_training_opts.mle && trainFlag
        ...
        gp_training_opts.kernel_params.sigma = (gprMdl.KernelInformation.KernelParameters(end))^2;  %kernel variance
        gp_training_opts.kernel_params.theta_vec = 1./(2*(gprMdl.KernelInformation.KernelParameters(1:(end-1))).^2);   %lengthscales
        gp_training_opts.kernel_params.sigma_p = gprMdl.Sigma;  %noise std
