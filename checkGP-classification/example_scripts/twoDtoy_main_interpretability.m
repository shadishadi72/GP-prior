...
dirName = strcat('results/',datestr(datetime('now')),'Interpretability_toy2D');
...
%% Summary of results
plot2dInterpretability(classA,classB,pis,pi_UUs_a,pi_UUs_m,pi_LLs_a,...
    pi_LLs_m,pi_UUs_a2,pi_UUs_m2,pi_LLs_a2,pi_LLs_m2,limecoefs,...
    params_for_gp_toolbox,saving_flag,epsilons,dirName,testIdx)
...
results.pis = pis;
results.pi_LLs_a = pi_LLs_a;
results.pi_UUs_a = pi_UUs_a;
results.pi_Us_s = pi_Us_s;
results.pi_Ls_s = pi_Ls_s;

if saving_flag
    save(strcat(dirName,'/ParamsAndSettings.mat'),'saveables');
    save(strcat(dirName,'/Results.mat'),'results');
end
